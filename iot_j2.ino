...
long myChannelNumber = 		;                                           // ThingSpeak channel
const char myWriteAPIKey[] = 				;                          // ThingSpeak write key
...
void setup() {
 ...
 WiFi.begin("", "");                                     // Connect to WiFi using given id and password
...
void loop() {
  ...
  sensorValue2 = map(sensorValue2, 0 , 1023, 1023, 0);                    // Invert the range
...
ThingSpeak.writeField(myChannelNumber, 3, 1, myWriteAPIKey);          // Send 1 to ThingSpeak in channel 3
//delay(100);
